...
int nonblock(int fd) {
	...
	check(flags >= 0 , "invalid falgs on nonblock");
...
int write_some(RingBuffer *buffer , int fd , int is_sock)
{
	...
	check(bfindreplace(data , &NL , &CRLF , 0) == BSTR_OK , "failed to replace");
...
int main(int argc , char *argv[])
{
	...
	while(1) {
		...
		while( !RingBuffer_empty(in_rb)){
			rc = write_some(sock_rb , socket , 1);
			check_debug(rc != -1 , "failed towrite to socket");
		}
	}
